...
public class BigNumber implements Serializable {
	 ...
	 */
		...
		/*
		 * перевод числа в систему счисления по основанию  pow(10, base). 
		 * Короче разбивает на фрагменты по base цифр
		 * Разворачивает число, ставя младший разряд в первый элемент массива
		 */
		...
		/*
		  *нормализация числа.
		  * число нормализуется по оснавию pow(10, base)
		  */
		private void normalization(){
			 int divinc = 0; // переменная которая суммируеться к старшему порядку, если текущий розряд число очазался больше основания 
			 int divdec = 0; // переменная которая суммируеться к старшему порядку, при сложении чисел, одно из которых отрицательное
			 long temp;
			 for (int i = 0; i < this.bits.length; i++){
		...
		/* 
		 * если эта переменная больше ноля, значит надо добавить ещё один розряд числа и записать туда значение divinc
		 * НАПРИМЕР если добавить 9+9  то в результате будет 8 а 1 пойдет в старший розряд чилса. В итоге получаеться 18
		 */	
			 if (divinc > 0){
		...
		/* 
		 * сравнивает текущее большое число с большим числом  arg0
		 * если текущее больще arg0 возвращает 1, если меньше - -1,
		 * если они равны - 0
		 */
		 ...
		 /*
		  * суммирование двох больших чисел. 
		  * arg0 - число которое суммируеться к текущему
		  * метод возвращает новый объект класса BigNumber в котором и находится сумма.
		  */
		 ...
		 /*
		  * Вычитание arg0 с текущего большого числа.
		  * Метод возвращает объект типа BigNumber 
		  */
		 ...
		 /*
		  * Сдвиг числа на count разрядов влево без потери старших разрядов.
		  * Фактически умножение на 10^base*count.
		  */
		 ...
		 /*
		  * Вырезает часть разрядов с позиции start(включительно) по позицию edn(включительно)
		  * с числа в фрагментированом виде. Начало - старший розряд
		  */
		 ...
		 /*
		  * Медот увеличивает текущее число на 1. 
		  * метод МОДИФИЦИРУЕТ this ОБЪЕКТ
		  */
		 BigNumber inc(){
			 bits[0]++;
			 if (this.bits[0]>Math.pow(10, BASE))
		...
		/*
		 * умноение текущего числа на arg0 методом Карацубы
		...
		// деление . Флаг указывает что вернуть, отсаток или целую часть.
		 ...
		 static boolean miller_rabin(BigNumber arg0, int raund, int begin){
			 ...
			 //Каноничекое разложение числа arg0 к виду arg0-1 = 2^s*t
			    int s = 0;
